...
### Writing **Your First Deployment** in Will's voice
...
small local Kubernetes setup with Minikube. Structurally, the script

- builds the docker image we'll use
- loads that image into Minikube
- replaces any old deployment with your current YAML files, which starts the stack
- waits until the PostgreSQL DB is stable and pods are up and healthy
- prints some information about the stack
- runs the `test-api.sh` script
- notifies you of success and tells you how to set up port-forwarding manually

Simply running this script and watching the messages it produces is fairly illuminating.
Useful, important stuff is happening, but it's all pretty straightforward, nothing magical
or incomprehensible going on.

### Sidebar: A Real Application Bug, and a Local-Registry Trap

Symptom: `./test-api.sh` failed on "Create item" with a 500, right after health/list/get all passed.

---

**Root cause #1** — a Python logging collision. In `app.py`, `create_item` logged:

```python
logger.info("item created", extra={"item_id": item.id, "name": item.name, "value": item.value})
```

Python's logging module reserves name as a built-in LogRecord attribute — it's the logger's own name. Passing name inside extra={} collides with that, and makeRecord raises:

```python
KeyError: "Attempt to overwrite 'name' in LogRecord"
```

Every other log call in the file was fine by coincidence — this was the one
place logging a field that happened to be called name, because that's the
natural field name on the Item model. Fix: rename the extra key to `item_name`.
The same bug existed in `update_item` at the same line pattern and got the same
fix.

**Lesson**: when you build a custom JSON log formatter (this repo has one),
watch for field names that shadow LogRecord's reserved attributes (name, msg,
args, levelname, module, filename, etc.) — a passing test suite that never
exercises the exact log line won't catch it, and it only surfaces as a 500 with
a stack trace pointing at the logging call, not the business logic, which
briefly looks like the wrong file entirely.

---

**Root cause #2** — a stale local image, hidden by a mutable tag. After fixing
the code and running docker build + minikube image load, the bug was still
there. Tracing it down:

```shell
kubectl exec <pod> -- grep -n "item_name" /app/app.py   # nothing — old code still running
minikube ssh -- "docker images k8s-toy-api --no-trunc"   # digest didn't match the fresh build
```

`minikube image load` silently no-op'd because a container was still running
against the old image under the same tag (`k8s-toy-api:local`). Deleting the
pod didn't help either — the Deployment immediately recreated a new pod from
the same stale cached image before the old one could be removed.

Fix — scale to zero before swapping the image:

```shell
kubectl exec <pod> -- grep -n "item_name" /app/app.py   # nothing — old code still running
kubectl scale deployment/toy-api --replicas=0
kubectl wait --for=delete pod -l app=toy-api --timeout=30s
minikube image rm k8s-toy-api:local
minikube image load k8s-toy-api:local
kubectl scale deployment/toy-api --replicas=2
```

**Lesson**: `imagePullPolicy: IfNotPresent` with a mutable tag like `:local` is
exactly the setup that makes "I rebuilt the image but the cluster is still
running old code" a recurring trap in local K8s dev. The tag not changing means
nothing forces a refresh; you have to explicitly evict every running consumer
before the old image can be replaced. This is one of the concrete reasons real
deployments use immutable, content-addressed tags (a git SHA or build digest)
instead of `:latest` or `:local` — the tag itself becomes proof of what's
running, and this whole class of bug can't happen.
